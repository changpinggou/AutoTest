...
def zip_folder(source_folder, dest_folder, zip_name, exclude_file_names: List[str]=['.DS_Store']):
    ...
    for tar in filelist:
        ...
        _write_zip_content(tar, f, filename)
...
def zip_folder_list(src_folder_list: List[str], dst_folder: str, zip_name: str, exclude_file_names: List[str]=['.DS_Store'], append_dir_link: bool=True):
    def _is_exclude_file(fname):
        if exclude_file_names is None or len(exclude_file_names) == 0:
            return False
        for exclude_name in exclude_file_names:
...
def _write_zip_content(src_file, zip_fh, name_in_zip):
...
def unzip_file(src_zip_file: str, dst_folder: str):
    ...
    Note: Also support tar/tgz/tar.gz
    ...
    if src_zip_file.endswith('.tar') or src_zip_file.endswith('.gz') or src_zip_file.endswith('.tgz'):
...
if __name__ == '__main__':
    zip_single_file(__file__, '.', 'tmp1.zip')
    zip_folder("lib", ".", "tmp2.zip")
    zip_folder_list([".git", "lib"], ".", "tmp3.zip")
    unzip_file("tmp3.zip", "__tmp__")
