...
def get_ios_catalyst_build_command(home, target, config):
  # Find project name
  proj_file = None
  for file_item in os.listdir(home):
    if file_item.endswith('.xcodeproj'):
      proj_file = file_item
      break

  # use ALL_BUILD scheme if not specified a target
  scheme = target
  if not scheme:
    scheme = 'ALL_BUILD'

  # WARN: Many system APIs (such as camera) were only supported since 14.0, so we drop the support of 13.0 (macOS 10.15)
  build_command = ['xcodebuild', '-project', '{0}/{1}'.format(home, proj_file), '-scheme', scheme, '-destination', 'platform=macOS,variant=Mac Catalyst', '-configuration', config, 'IPHONEOS_DEPLOYMENT_TARGET=14.0']
...
parser.add_argument(
    '--ios-variant',
    choices=['device', 'simulator', 'maccatalyst'],
    help="Build which iOS variant"
)
parser.add_argument(
    '--force-use-xcode-legacy',
    action='store_true',
    help="Since CMake 3.19 uses Xcode new build system by default, set this to continue using legacy build system"
)
parser.add_argument(
    '--framework-support-multi-variant',
    action='store_true',
    help="Whether append a sub folder 'ios-variant' under '_framework/[toolchain]' folder or not"
)
parser.add_argument(
    '--no-codesign',
    action='store_true',
    help="Whether not to codesign the framework product"
)
parser.add_argument(
    '--jobs',
    type=int,
...
if args.framework_support_multi_variant:
  build_dir = os.path.join(cdir, '_builds', build_tag, args.ios_variant)
  install_dir = os.path.join(cdir, '_install', polly_toolchain, args.ios_variant)
  framework_dir = os.path.join(cdir, '_framework', polly_toolchain, args.ios_variant)
  archives_dir = os.path.join(cdir, '_archives', args.ios_variant)
else:
...
if args.config:
  generate_command.append("-DZEGO_GN_BUILD_TYPE={}".format(args.config.lower()))

if args.ios_variant == "maccatalyst":
  generate_command.append('-Tbuildsystem=12')
elif args.force_use_xcode_legacy:
  generate_command.append('-Tbuildsystem=1')
...
if toolchain_entry.vs_version:
  # On Visual Studio 16 2019, the CMake require "-A" to specify arch
  if toolchain_entry.vs_version == '16':
    vs_arch = ''
    if toolchain_entry.arch == 'x86': vs_arch = 'Win32'
    if toolchain_entry.arch == 'amd64': vs_arch = 'x64'
    if vs_arch:
      generate_command.append('-A{}'.format(vs_arch))
...
if args.ios_combined and args.ios_variant != "maccatalyst":
...
if args.ios_variant == 'device':
  generate_command.append("-DIOS_VARIANT=DEVICE")
elif args.ios_variant == 'simulator':
  generate_command.append("-DIOS_VARIANT=SIMULATOR")
elif args.ios_variant == 'maccatalyst':
  generate_command.append("-DIOS_VARIANT=MACCATALYST")
  generate_command.append("-DSUPPORTS_MACCATALYST=YES")
...
if args.iossim or args.ios_variant == 'simulator':
  build_command.append('-sdk')
  build_command.append('iphonesimulator')
elif args.ios_variant == 'maccatalyst':
  # build_command.append('-sdk')
  # build_command.append('macosx')
  pass
...
if not args.nobuild:
  ...
  if toolchain_entry.is_xcode and args.ios_variant == "maccatalyst":
    build_command = get_ios_catalyst_build_command(build_dir, install_target_name, args.config)
