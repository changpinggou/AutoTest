...
def tee(infile, discard, logging, console=None):
  ...
  def fanout():
    ...
    for line in iter(infile.readline, b''):
      ...
      try: s = line.decode(encoding=encoding, errors='replace')
      except:
        try: s = line.decode(encoding=sys.stdout.encoding, errors='replace')
        except:
          try: s = line.decode(encoding='utf8', errors='replace')
          except: continue # Unknown encoding, just skip it
      s = s.replace('\r', '')
      s = s.replace('\t', '  ')
      ...
      try:
        ...
        print(line)
        continue # Ignore encoding error
      discard_counter += 1
