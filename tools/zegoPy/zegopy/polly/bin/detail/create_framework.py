...
def run(install_dir, framework_dir, ios, polly_root, device, logging, plist=None, identity=None, lib_regex='*', no_codesign=False):
  ...
  if ios:
    ...
    resources_dir = os.path.join(framework_dir, 'Versions', 'A', 'Resources')
  ...
  if not ios:
    ...
    if len(plist) != 0:
      framework_plist = os.path.join(resources_dir, "Info.plist")
